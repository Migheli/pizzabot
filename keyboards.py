...
def get_multipage_keyboard(products_datasets, index_of_page):
    ...
    if index_of_page > 0:
        back_products_button = InlineKeyboardButton(
            text='<<',
            callback_data=f'show_products_page::{index_of_page - 1}'
        )
        navigation_buttons.append(back_products_button)

    if index_of_page + 1 < len(products_datasets):
        next_products_button = InlineKeyboardButton(
            text='>>',
            callback_data=f'show_products_page::{index_of_page + 1}'
        )
...
def get_delivery_type_keyboard(nearest_entry_id, longitude, latitude):
    return [
        ...
        [
            InlineKeyboardButton(
                text='Доставьте',
                callback_data=f'shpg::{nearest_entry_id}::{longitude}::{latitude}'
            )
        ]
    ]
